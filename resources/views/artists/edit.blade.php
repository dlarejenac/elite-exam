...
<div class="max-w-xl mx-auto p-6 bg-white rounded-lg shadow-md mt-8">
    ...
    <form action="{{ route('artists.update', $artist) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
        <div>
            <label for="code" class="block mb-2 text-sm font-medium text-gray-900">Code</label>
            <input type="text" id="code" name="code" value="{{ old('code', $artist->code) }}"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-yellow-500 focus:border-yellow-500 w-full p-2.5 @error('code') border-red-500 @enderror" required>
            @error('code')
        ...
        <div>
            ...
            <input type="text" id="name" name="name" value="{{ old('name', $artist->name) }}"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-yellow-500 focus:border-yellow-500 w-full p-2.5 @error('name') border-red-500 @enderror" required>
            @error('name')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
